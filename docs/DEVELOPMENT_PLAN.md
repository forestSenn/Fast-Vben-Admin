...
| v2.0 平台化 | 部分完成 | 多租户主链路、五级数据权限和可配置初始化模板已完成；可选 BPM 最小 POC 已实现并默认关闭，正式引入仍待许可证和运维评审；Docker 实机验收待推进 |
...
| v2.0 | 多租户、数据权限、可选 BPM | 部分完成 | 20-35 天起 | 多租户与数据权限主链路已完成，初始化模板和发布环境验收待收口 |
...
当前状态（2026-07-15）：独立 POC 已使用 SpiffWorkflow 3.1 实现，运行时由 `BPM_ENABLED` 控制且默认关闭。定义/版本发布、发起、审批、驳回、撤回、转交、抄送、四类组织表达式、业务表单关联、审计、超时标记、通知记录和跨租户阻断已有后端自动化测试，前端提供独立流程工作台。SpiffWorkflow 使用 LGPLv3，正式启用前仍需完成许可证、调度、监控、持久化兼容和容量评审。
...
3. 对 BPM POC 完成许可证、调度、监控、备份恢复和容量评审，再决定是否正式启用。
